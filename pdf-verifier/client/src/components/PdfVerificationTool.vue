<template>
    <Card header="Outil de vérification PDF" class="mb-5">
      <template #content>
        ...
        <Button
          ...
          class="mt-3"
          :loading="isLoading"
          :disabled="isLoading || !pdfFile || !selectedTests || selectedTests.length === 0"
          @click="analyzePdf"
        />
      </template>
    </Card>
    ...
    <!-- Résultats des tests -->
    <ResultTable :results="analysisResult" />
  </template>
  ...
  import usePdfAnalyzer from '@/composables/usePdfAnalyzer.js';
  ...
  function onFileSelect(event) {
    if (event.files.length > 0) {
      ...
      // Remplacer le tableau de fichiers par le fichier sélectionné
  ...
  async function analyzePdf() {
    ...
    await analyzePdfFileBackend(props.pdfFile);
  }
  </script>
